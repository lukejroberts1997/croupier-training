(function () {
  ...
  function buildScenario() {
    ...
    for (let i = 0; i < 4; i++) {
      ...
      let folds = 0;
      if (players > 2) {
        const maxFolds = Math.min(players - 1, 3);
      ...
      const bet = players >= 2 ? randomChoice(betOptions) : 0;
  ...
  function renderScenario(scenario) {
    ...
    playersInfoEl.innerHTML =
      "<strong>" +
      scenario.totalPlayers +
      " players</strong> are seated at the table.";
  ...
  function showResults(allCorrect, details) {
    ...
    fields.forEach((f) => {
      ...
      if (isRight) {
        detailHtml +=
          '<div><span class="correct-val">\u2713</span> ' +
          labels[f] +
          ": " +
          correct +
          " kr</div>";
        ...
        detailHtml +=
          '<div><span class="incorrect-val">\u2717</span> ' +
          labels[f] +
          ": You said " +
          (answer !== null ? answer + " kr" : "(empty)") +
          " — correct is <strong>" +
          correct +
          " kr</strong></div>";
      }
    ...
    detailHtml +=
      "<div><strong>Winner Receives: " +
      currentScenario.payout +
      " kr</strong></div>";
